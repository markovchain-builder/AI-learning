...
# AIMessages, HumanMessages, SystemMessages 都是BaseMessage的子类
...
class FileChatMessageHistory(BaseChatMessageHistory):
    ...
    def add_messages(self, messages:Sequence[BaseMessage]) -> None:
        # Sequence类似于list,tuple
        all_messages=list(self.messages) #已有的消息列表
        all_messages.extend(messages)  #新的和已有 的融合成一个list
        
        #将数据同步写入本地文件中
        #类对象写入文件中 -> 一堆二进制
        #为了方便，可以将BaseMessage消息转为字典(借助json模块以json字符串接入文件)
    ...
    def clear(self) -> None:
        with open(self.file_path,"w",encoding_path="utf-8") as f:
            json.dump([],f)
...
if __name__ == '__main__':
    ...
    # res=conversation_chain.invoke({"input":"小刚有3只龟"},session_config)
    # print("第二次执行",res)
